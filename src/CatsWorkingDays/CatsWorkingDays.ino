...
#include "screens.h"      // полноэкранные картинки: меню, контролы, game over, ранги, анлоки способностей
...
// Картинка ранга — по тем же порогам, что и rankTitle().
// ARCHITECT пока переиспользует картинку SENIOR (отдельного арта нет).
const uint8_t* rankScreenBg(uint16_t blocked) {
  if (blocked < 10) return firedScreenBg;
  if (blocked < 15) return juniorEndBg;
  if (blocked < 20) return middleEndBg;
  return seniorEndBg;
}
...
void loop() {
  ...
  switch (currentState) {
    ...
    // ---------------- МЕНЮ ----------------
    case STATE_MENU:
      arduboy.drawBitmap(0, 0, startScreenBg, SCREEN_W, SCREEN_H, WHITE);
      arduboy.setCursor(4, 48);  arduboy.print("A:START");
      arduboy.setCursor(64, 48); arduboy.print("B:CONTROLS");
    ...
    // ---------------- КОНТРОЛС ----------------
    case STATE_CONTROLS:
      arduboy.drawBitmap(0, 0, controlsScreenBg, SCREEN_W, SCREEN_H, WHITE);
    ...
    case STATE_LEVELUP:
      // На 2 уровне открывается способность A, на 3-м — способность B
      if (currentLevel + 1 == 2) {
        arduboy.drawBitmap(0, 0, abilityAUnlockBg, SCREEN_W, SCREEN_H, WHITE);
      } else if (currentLevel + 1 == 3) {
        arduboy.drawBitmap(0, 0, abilityBUnlockBg, SCREEN_W, SCREEN_H, WHITE);
      } else {
    ...
    case STATE_RANK:
      arduboy.drawBitmap(0, 0, rankScreenBg(blockedCount), SCREEN_W, SCREEN_H, WHITE);
      arduboy.setCursor(2, 56); arduboy.print("Blocked: "); arduboy.print(blockedCount);
    ...
    // ---------------- GAME OVER ----------------
    case STATE_GAMEOVER:
      arduboy.drawBitmap(0, 0, gameOverScreenBg, SCREEN_W, SCREEN_H, WHITE);
      arduboy.setCursor(2, 54); arduboy.print("SCORE:"); arduboy.print(score);
